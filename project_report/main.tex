...
Face anti-spoofing is an important problem in face recognition system. Although most face anti-spoofing methods use the hand-crafted texture features, e.g., LBP, LBP-TOP, but we decide to use deep convolutional neural network (CNN), which can capture the most discriminative features between genuine and fake faces. Combined with some data pre-processing, the CNN method achieved pretty good results on the given database. In the experiments, 98.46\% accuracy is achieved on the Idiap REPLAY-ATTACK database, which is comparable to the state-of-the-art methods. 
...
\IEEEPARstart{F}{ace} anti-spoofing, as a security measure for face recognition system, are drawing increasing attentions in both academic and industrial fields. Due to the diversity of spoofing types, including print-attacks, replay-attacks, mask-attacks, etc., itis still a difficult work to distinguish various fake faces. Usually, there is no visual cue that is available to us to pick the genuine face images.
...
The remainder of this paper is organized as following. In section \uppercase\expandafter{\romannumeral2}, our innovation and the approach to detect face liveness are introcduced in detail. Then in section \uppercase\expandafter{\romannumeral3}, the experiments are presented and the results are discussed. Finally, in section \uppercase\expandafter{\romannumeral4}, a conclusion is drawn.
...
\begin{figure}[!t]
	...
	\subfigure{}{
		\begin{minipage}{4cm}
		     \centering
		     \includegraphics[width=4cm]{figure/EX_1}
		     \end{minipage}
	}
	\subfigure{}{
		\begin{minipage}{4cm}
			\centering
			\includegraphics[width=4cm]{figure/EX_2}
		\end{minipage}
	}
    \subfigure{}{
	\begin{minipage}{4cm}
		\centering
		\includegraphics[width=4cm]{figure/EX_3}
	\end{minipage}
    }
...
\end{equation}
 ...
 \begin{table}[ht]
 	...
 	\normalsize
 	\centering
 	\caption{ HTER ($\%$) of classiﬁcation for the Replay Attack dataset}
 	\label{tab_EX_2}
 	\begin{tabular}{|c|c|}
 		\hline
 		Method & Test ($\%$)\\
